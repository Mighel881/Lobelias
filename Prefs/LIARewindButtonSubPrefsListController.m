#import "LIARewindButtonSubPrefsListController.h"

BOOL enableRewindButtonSection = NO;

UIBlurEffect* blur;
UIVisualEffectView* blurView;

@implementation LIARewindButtonSubPrefsListController
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier*)specifier{
    ...
    if ([specifier.properties[@"key"] isEqualToString:@"customRewindButtonPositionAndSize"] && [value isEqual:@(NO)]) {
    ...
    } else if ([specifier.properties[@"key"] isEqualToString:@"customRewindButtonPositionAndSize"] && [value isEqual:@(YES)]) {
...
- (void)toggleState {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:@"/var/mobile/Library/Preferences/love.litten.lobeliaspreferences.plist"]) {
        enableRewindButtonSection = YES;
        [preferences setBool:enableRewindButtonSection forKey:@"EnableRewindButtonSection"];
        [self toggleCellState:YES];
    } else if (![allKeys containsObject:@"EnableRewindButtonSection"]) {
        enableRewindButtonSection = YES;
        [preferences setBool:enableRewindButtonSection forKey:@"EnableRewindButtonSection"];
        [self toggleCellState:YES];
    } else if ([[preferences objectForKey:@"EnableRewindButtonSection"] isEqual:@(NO)]) {
        enableRewindButtonSection = YES;
        [preferences setBool:enableRewindButtonSection forKey:@"EnableRewindButtonSection"];
        [self toggleCellState:YES];   
    } else if ([[preferences objectForKey:@"EnableRewindButtonSection"] isEqual:@(YES)]) {
        enableRewindButtonSection = NO;
        [preferences setBool:enableRewindButtonSection forKey:@"EnableRewindButtonSection"];
...
- (void)setCellsHidden {
    ...
    if ([[preferences objectForKey:@"customRewindButtonPositionAndSize"] isEqual:@(YES)]) {
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:4 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:6 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:8 inSection:0] enabled:YES];
