#import "LIATimeControlSubPrefsListController.h"

BOOL enableTimeControlSection = NO;

UIBlurEffect* blur;
UIVisualEffectView* blurView;

@implementation LIATimeControlSubPrefsListController
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier*)specifier{
    ...
    if ([specifier.properties[@"key"] isEqualToString:@"customTimeControlPositionAndSize"] && [value isEqual:@(NO)]) {
    ...
    } else if ([specifier.properties[@"key"] isEqualToString:@"customTimeControlPositionAndSize"] && [value isEqual:@(YES)]) {
...
- (void)toggleState {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:@"/var/mobile/Library/Preferences/love.litten.lobeliaspreferences.plist"]) {
        enableTimeControlSection = YES;
        [preferences setBool:enableTimeControlSection forKey:@"EnableTimeControlSection"];
        [self toggleCellState:YES];
    } else if (![allKeys containsObject:@"EnableTimeControlSection"]) {
        enableTimeControlSection = YES;
        [preferences setBool:enableTimeControlSection forKey:@"EnableTimeControlSection"];
        [self toggleCellState:YES];
    } else if ([[preferences objectForKey:@"EnableTimeControlSection"] isEqual:@(NO)]) {
        enableTimeControlSection = YES;
        [preferences setBool:enableTimeControlSection forKey:@"EnableTimeControlSection"];
        [self toggleCellState:YES];   
    } else if ([[preferences objectForKey:@"EnableTimeControlSection"] isEqual:@(YES)]) {
        enableTimeControlSection = NO;
        [preferences setBool:enableTimeControlSection forKey:@"EnableTimeControlSection"];
...
- (void)setCellsHidden {
    ...
    if ([[preferences objectForKey:@"customTimeControlPositionAndSize"] isEqual:@(YES)]) {
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:4 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:6 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:8 inSection:0] enabled:YES];
