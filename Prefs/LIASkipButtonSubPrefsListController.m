#import "LIASkipButtonSubPrefsListController.h"

BOOL enableSkipButtonSection = NO;

UIBlurEffect* blur;
UIVisualEffectView* blurView;

@implementation LIASkipButtonSubPrefsListController
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier*)specifier{
    ...
    if ([specifier.properties[@"key"] isEqualToString:@"customSkipButtonPositionAndSize"] && [value isEqual:@(NO)]) {
    ...
    } else if ([specifier.properties[@"key"] isEqualToString:@"customSkipButtonPositionAndSize"] && [value isEqual:@(YES)]) {
...
- (void)toggleState {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:@"/var/mobile/Library/Preferences/love.litten.lobeliaspreferences.plist"]) {
        enableSkipButtonSection = YES;
        [preferences setBool:enableSkipButtonSection forKey:@"EnableSkipButtonSection"];
        [self toggleCellState:YES];
    } else if (![allKeys containsObject:@"EnableSkipButtonSection"]) {
        enableSkipButtonSection = YES;
        [preferences setBool:enableSkipButtonSection forKey:@"EnableSkipButtonSection"];
        [self toggleCellState:YES];
    } else if ([[preferences objectForKey:@"EnableSkipButtonSection"] isEqual:@(NO)]) {
        enableSkipButtonSection = YES;
        [preferences setBool:enableSkipButtonSection forKey:@"EnableSkipButtonSection"];
        [self toggleCellState:YES];   
    } else if ([[preferences objectForKey:@"EnableSkipButtonSection"] isEqual:@(YES)]) {
        enableSkipButtonSection = NO;
        [preferences setBool:enableSkipButtonSection forKey:@"EnableSkipButtonSection"];
...
- (void)setCellsHidden {
    ...
    if ([[preferences objectForKey:@"customSkipButtonPositionAndSize"] isEqual:@(YES)]) {
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:4 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:6 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:8 inSection:0] enabled:YES];
