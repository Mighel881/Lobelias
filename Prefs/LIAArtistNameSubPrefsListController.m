#import "LIAArtistNameSubPrefsListController.h"

BOOL enableArtistNameSection = NO;

UIBlurEffect* blur;
UIVisualEffectView* blurView;

@implementation LIAArtistNameSubPrefsListController
...
- (void)setPreferenceValue:(id)value specifier:(PSSpecifier*)specifier{
    ...
    if ([specifier.properties[@"key"] isEqualToString:@"customArtistNamePositionAndSize"] && [value isEqual:@(NO)]) {
    ...
    } else if ([specifier.properties[@"key"] isEqualToString:@"customArtistNamePositionAndSize"] && [value isEqual:@(YES)]) {
...
- (void)toggleState {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:@"/var/mobile/Library/Preferences/love.litten.lobeliaspreferences.plist"]) {
        enableArtistNameSection = YES;
        [preferences setBool:enableArtistNameSection forKey:@"EnableArtistNameSection"];
        [self toggleCellState:YES];
    } else if (![allKeys containsObject:@"EnableArtistNameSection"]) {
        enableArtistNameSection = YES;
        [preferences setBool:enableArtistNameSection forKey:@"EnableArtistNameSection"];
        [self toggleCellState:YES];
    } else if ([[preferences objectForKey:@"EnableArtistNameSection"] isEqual:@(NO)]) {
        enableArtistNameSection = YES;
        [preferences setBool:enableArtistNameSection forKey:@"EnableArtistNameSection"];
        [self toggleCellState:YES];   
    } else if ([[preferences objectForKey:@"EnableArtistNameSection"] isEqual:@(YES)]) {
        enableArtistNameSection = NO;
        [preferences setBool:enableArtistNameSection forKey:@"EnableArtistNameSection"];
...
- (void)setCellsHidden {
    ...
    if ([[preferences objectForKey:@"customArtistNamePositionAndSize"] isEqual:@(YES)]) {
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:4 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:6 inSection:0] enabled:YES];
        [self setCellForRowAtIndexPath:[NSIndexPath indexPathForRow:8 inSection:0] enabled:YES];
